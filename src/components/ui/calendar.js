...
function Calendar(_a) {
    ...
    return (<react_day_picker_1.DayPicker showOutsideDays={showOutsideDays} className={(0, utils_1.cn)("p-3", className)} classNames={__assign({ months: "flex flex-col sm:flex-row gap-2", month: "flex flex-col gap-4", caption: "flex justify-center pt-1 relative items-center w-full", caption_label: "text-sm font-medium", nav: "flex items-center gap-1", nav_button: (0, utils_1.cn)((0, button_1.buttonVariants)({ variant: "outline" }), "size-7 bg-transparent p-0 opacity-50 hover:opacity-100"), nav_button_previous: "absolute left-1", nav_button_next: "absolute right-1", table: "w-full border-collapse space-x-1", head_row: "flex", head_cell: "text-muted-foreground rounded-md w-8 font-normal text-[0.8rem]", row: "flex w-full mt-2", cell: (0, utils_1.cn)("relative p-0 text-center text-sm focus-within:relative focus-within:z-20 [&:has([aria-selected])]:bg-accent [&:has([aria-selected].day-range-end)]:rounded-r-md", props.mode === "range"
            ...
            IconLeft: function (_a) {
                ...
                return (<lucide_react_1.ChevronLeft className={(0, utils_1.cn)("size-4", className)} {...props}/>);
            },
            IconRight: function (_a) {
                var className = _a.className, props = __rest(_a, ["className"]);
                return (<lucide_react_1.ChevronRight className={(0, utils_1.cn)("size-4", className)} {...props}/>);
            },
        }} {...props}/>);
}
